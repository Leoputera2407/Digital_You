...
_VALID_FILE_EXTENSIONS = [".md", ".markdown", ".pdf", ".html",
                          ".pptx", ".ppt", ".docx", ".doc", ".epub", ".txt", ".zip"]
file_processors = {
    ".txt": TextLoader,
...
def write_temp_files(
    ...
    for file_name, file in files:
        ...
        res = supabase.storage.from_(BUCKET_NAME).upload(
            bucket_file_path, file_content, upsert=True)

        if 'error' in res:
            raise Exception(
                f"Error uploading file to Supabase: {res['error']['message']}")
        ...
        with tempfile.NamedTemporaryFile(suffix=".zip", delete=True) as temp_file:
            ...
            for file_name, file in get_files_from_zip(temp_file.name):
                ...
                for doc in langchain_docs:
                    yield Document(
                        id=file_name,
                        sections=[Section(link=None, text=doc.page_content)],
