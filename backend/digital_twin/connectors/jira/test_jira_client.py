...
class JiraClient:
    ...
    def get_issues_and_comments(
        ...
        start_date_str = datetime.fromtimestamp(start, tz=timezone.utc).strftime("%Y-%m-%dT%H:%M")
        end_date_str = datetime.fromtimestamp(end, tz=timezone.utc).strftime("%Y-%m-%dT%H:%M")
