...
from digital_twin.connectors.github.model import Label, Person, PullRequest
    ...
    def _map_state_filter(state_filter: str) -> list[str]:
        ...
        state_map = {
            ...
            "closed": [
                "CLOSED",
                "MERGED",
            ],  # In GitHub API, 'closed' includes merged PRs
        }
        ...
        while True:
            ...
            for edge in response["data"]["repository"]["pullRequests"]["edges"]:
                ...
                pr_data = PullRequest(
                    ...
                    number=pr["number"],
                    created_at=datetime.fromisoformat(pr["createdAt"]).timestamp()
                    if pr["createdAt"]
                    else None,
                    updated_at=datetime.fromisoformat(pr["updatedAt"]).timestamp()
                    if pr["updatedAt"]
                    else None,
                    ...
                    reviewers=[
                        Person(name=r["author"]["login"]) for r in pr["reviews"]["nodes"] if r["author"]
                    ],
            ...
            if not response["data"]["repository"]["pullRequests"]["pageInfo"]["hasNextPage"] or (
                time_range_start is not None and last_modified < time_range_start
            ):
                break
