...
from digital_twin.db.connectors.connectors import async_fetch_connector_by_id_and_org
from digital_twin.db.connectors.credentials import async_fetch_credential_by_id_and_org
from digital_twin.db.model import Connector, ConnectorCredentialPair, Credential, IndexingStatus, User
...
async def async_fetch_connector_by_org_id(
    ...
    stmt = (
        ...
        .join(ConnectorAlias, ConnectorCredentialPair.connector_id == ConnectorAlias.id)
        .join(CredentialAlias, ConnectorCredentialPair.credential_id == CredentialAlias.id)
        .where(
            and_(
    ...
    if not include_disabled:
        stmt = stmt.where(ConnectorAlias.disabled == False)
...
async def async_get_connector_credential_pairs(
    ...
    if not include_disabled:
        stmt = stmt.where(ConnectorCredentialPair.connector.has(Connector.disabled == False))
    results = await db_session.scalars(stmt)
    return list(results.unique().all())
...
async def async_update_connector_credential_pair(
    ...
    try:
        cc_pair = await async_get_connector_credential_pair(
            connector_id, credential_id, organization_id, db_session
        )
        if not cc_pair:
            logger.warning(
                f"Attempted to update pair for connector id {connector_id} "
