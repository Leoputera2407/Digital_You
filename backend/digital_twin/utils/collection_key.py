...
from digital_twin.indexdb.qdrant.indexing import list_qdrant_collections
...
def get_unique_collection_keys() -> Tuple[str, str]:
    ...
    while check_typesense_collection_exist(
        typesense_collection_key
    ):  # Keep generating keys while the key exists
        typesense_collection_key = str(uuid4())

    while qdrant_collection_key in [collection.name for collection in list_qdrant_collections().collections]:
        qdrant_collection_key = str(uuid4())
